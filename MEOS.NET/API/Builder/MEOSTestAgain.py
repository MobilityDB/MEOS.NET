...
def transform_fn_args(args: str, fnname:str) -> str:
    if args.strip() == "void":
        return ""

    splitted = args.split(',')
    arg_pattern = r'(?:\s*const)?\s*([\w\s]+)\s+(\*{1,2})\s*(\w+)\s*'
...
def process_line(line: str) -> Optional[str] :
    ...
    if match:
        ...
        arg_list = transform_fn_args(match.group(5).strip(), fn_name)
...
if __name__ == "__main__":
    file_path = "/home/totojr/Externals/MobilityDB/meos/include/meos.h"

    print("using System.Runtime.InteropServices;\n")
    print("namespace MEOS.NET.API.Internal")
    print("{")
    print("\tpublic static class MEOSFunctions2")
    print("\t{")
